pipeline {
  ...
  environment {
    BOT_NAME = 'outreach_manager_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-outreach_manager"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/outreach_manager_bot/bin/outreach_manager_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyOutreachManager.Release.create()'"
          $RELEASE_BIN eval 'BotArmyOutreachManager.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyOutreachManager.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyOutreachManager.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_outreach_manager/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_outreach_manager/releases/start_erl.data)
        fi
