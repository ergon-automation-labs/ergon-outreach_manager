# Getting Started with bot_army_outreach_manager

This guide walks you through setting up Outreach Manager Bot for local development.
...
DATABASE_NAME=bot_army_outreach_manager_dev
```
...
Outreach Manager Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_outreach_manager/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_outreach_manager/skills/example.ex lib/bot_army_outreach_manager/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyOutreachManager.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_outreach_manager.command.classify"]
...
```elixir
# In lib/bot_army_outreach_manager/application.ex or similar

defmodule BotArmyOutreachManager.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyOutreachManager.Skills.Classify,
      BotArmyOutreachManager.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_outreach_manager/skills/my_skill.ex
...
This creates a release named `outreach_manager_bot` that is deployed to `/opt/ergon/releases/outreach_manager_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/outreach_manager_bot/`
